...
### Step 1 — Scaffold the Project Architecture
...
#### 1.5 — Configure package.json for Publishing
...
Based on the Gemini.md and the previous instructions, build the new component [IconButton].

IconButton must follow the exact folder structure from `GEMINI.md`:
...
**Styling:**
- All styles via `IconButton.module.css`
- All color, spacing, radius, and motion values pulled from CSS custom properties — no hardcoded values
- Smooth hover and focus transitions using motion tokens
- Dark mode works automatically via theme tokens
...
1. Export the `IconButton` component and its types from `src/index.ts`
2. Confirm the project builds with zero TypeScript errors
3. Confirm all IconButton tests pass
4. Confirm the Storybook story renders correctly
